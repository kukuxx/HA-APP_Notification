...
class NotificationLogger(hass.Hass):
    ...
    async def save_notification(self, action_id, data):
        ...
        try:
            ...
            if image is None:
                notification = (
                    f"<ha-alert alert-type='info'>**{title}**</ha-alert>"
                    f"<p><blockquote>```{message}```<br>"
                    f"**_{send_time}_**</blockquote></p>"
                )
            else:
                notification = (
                    f"<ha-alert alert-type='info'>**{title}**</ha-alert>"
            ...
            if len(notifications_list) > 30:
                notifications_list.pop()  # 刪除最舊的通知
    ...
    async def update_notification_log(self, action_id):
        ...
        if notification_log is not None:
            ...
            self.set_state(
                ...
                attributes={"notifications": notification_log}
            )
        else:
